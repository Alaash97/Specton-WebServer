...
export async function handlePlayerMessage(
  ...
  switch (message.type) {
    ...
    case 'map_tile': {
      const session = sessionManager.getSessionByPlayer(ws);
      if (session) {
        const tile = message as any;
        void session
          .setMapTileAsset(
            String(tile.path),
            String(tile.mimeType ?? 'image/jpeg'),
            String(tile.data),
          )
          .then((accepted) => {
            if (!accepted) {
          ...
          .catch((error) => {
            console.warn(
              `[Player] Map tile upload failed for session "${session.sessionId}": ${error?.message ?? error}`,
            );
    ...
    case 'map_tiles': {
      ...
      if (session) {
        ...
        console.log(`[Player] Map tiles received for session "${session.sessionId}" (${(message as any).lods?.length ?? 0} LODs)`);
      }
      break;
    }
    case 'map_objects': {
    ...
    case 'projectile_spawn': {
      const session = sessionManager.getSessionByPlayer(ws);
      if (session) session.sendToSpectators(message);
      break;
    }
    ...
    case 'drop_spawn': {
      const session = sessionManager.getSessionByPlayer(ws);
      if (session) session.addDrop(message as any);
      break;
    }
    case 'drop_destroy': {
      const session = sessionManager.getSessionByPlayer(ws);
      if (session) session.removeDrop((message as any).id);
      break;
    }
    case 'damage_dealt': {
      const session = sessionManager.getSessionByPlayer(ws);
...
export function handlePlayerDisconnect(ws: WebSocket, sessionManager: SessionManager): void {
