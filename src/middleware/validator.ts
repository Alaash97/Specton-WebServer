...
export function describeInvalidMessage(raw: string): string {
  let parsed: any;
  try {
  ...
  if (!parsed || typeof parsed !== 'object') return 'Message root is not an object.';
  if (!VALID_TYPES.includes(type)) return `Unknown message type '${String(type)}'.`;

  switch (type) {
    case 'session_status':
      if (typeof parsed.phase !== 'string') return 'session_status.phase must be a string.';
      if (!VALID_SESSION_PHASES.includes(parsed.phase)) return `Unsupported session_status phase '${parsed.phase}'.`;
      return 'Invalid session_status payload.';
    case 'map_image':
      if (typeof parsed.data !== 'string' || parsed.data.length === 0) return 'map_image.data is missing or empty.';
      if (parsed.data.length > 10_000_000) return `map_image.data is too large (${parsed.data.length} chars).`;
      return 'Invalid map_image payload.';
    case 'map_tile':
      if (typeof parsed.path !== 'string' || parsed.path.length === 0) return 'map_tile.path is missing.';
      if (typeof parsed.data !== 'string' || parsed.data.length === 0) return 'map_tile.data is missing or empty.';
      if (parsed.data.length > 25_000_000) return `map_tile.data is too large (${parsed.data.length} chars).`;
...
export function validateMessage(raw: string): ClientMessage | null {
  ...
  switch (parsed.type) {
    case 'create_session':
      if (typeof parsed.playerName !== 'string' || !parsed.playerName.trim()) return null;
      if (typeof parsed.level !== 'string' || !parsed.level.trim()) return null;
      if (parsed.playerName.length > 32 || parsed.level.length > 32) return null;
      return {
        type: 'create_session',
    ...
    case 'join_session':
      ...
      if (typeof parsed.spectatorName !== 'string' || !parsed.spectatorName.trim()) return null;
      if (parsed.spectatorName.length > 24) return null;
      if (
        parsed.spectatorToken !== undefined &&
    ...
    case 'rename_spectator':
      if (typeof parsed.sessionId !== 'string' || !parsed.sessionId.trim()) return null;
      if (typeof parsed.requestId !== 'string' || !parsed.requestId.trim() || parsed.requestId.length > 96) return null;
      if (typeof parsed.spectatorName !== 'string' || !parsed.spectatorName.trim() || parsed.spectatorName.length > 24) return null;
      return {
    ...
    case 'action':
      if (typeof parsed.sessionId !== 'string' || !parsed.sessionId.trim()) return null;
      if (typeof parsed.requestId !== 'string' || !parsed.requestId.trim() || parsed.requestId.length > 96) return null;
      if (typeof parsed.action !== 'string' || !parsed.action.trim() || parsed.action.length > 64) return null;
      return {
        type: 'action',
        sessionId: parsed.sessionId.trim(),
        requestId: parsed.requestId.trim(),
    ...
    case 'game_state':
      if (typeof parsed.playerX !== 'number' || typeof parsed.playerZ !== 'number') return null;
      return parsed as ClientMessage;

    case 'finish_game':
      if (typeof parsed.playerDeaths !== 'number' || !Number.isFinite(parsed.playerDeaths)) return null;
      return {
        type: 'finish_game',
    ...
    case 'map_image':
      ...
      if (parsed.data.length > 10_000_000) return null; // max ~7.5MB image
      return parsed as ClientMessage;

    case 'map_tile':
      if (typeof parsed.path !== 'string' || !parsed.path.trim()) return null;
      if (!/^[a-zA-Z0-9_./-]+$/.test(parsed.path) || parsed.path.includes('..')) return null;
      if (typeof parsed.data !== 'string' || parsed.data.length === 0) return null;
      if (parsed.data.length > 25_000_000) return null;
      return {
        type: 'map_tile',
    ...
    case 'map_tiles':
      if (!Array.isArray(parsed.lods)) return null;
      return parsed as ClientMessage;
    ...
    case 'ability_manifest':
      if (!Array.isArray(parsed.abilities)) return null;
      return parsed as ClientMessage;

    case 'weapon_manifest':
      if (!Array.isArray(parsed.weapons)) return null;
      return parsed as ClientMessage;
    ...
    case 'session_status':
      ...
      if (!VALID_SESSION_PHASES.includes(parsed.phase)) return null;
      return {
        type: 'session_status',
        phase: parsed.phase,
    ...
    case 'drop_spawn':
      if (typeof parsed.id !== 'string' || !parsed.id) return null;
      if (typeof parsed.dropType !== 'string') return null;
      return parsed as ClientMessage;
    ...
    case 'damage_dealt':
      if (typeof parsed.spectatorName !== 'string' || !parsed.spectatorName) return null;
      if (typeof parsed.damage !== 'number' || parsed.damage < 0) return null;
      return parsed as ClientMessage;
