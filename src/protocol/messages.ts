...
export interface GameStateMessage {
  ...
  playerMaxHP: number;
  playerDeaths?: number;
  enemies: Array<{ id: string; spriteId: string; x: number; y: number; z: number; rotY: number; hp: number; maxHp: number }>;
  mapMinX: number;
  mapMaxX: number;
  mapMinZ: number;
  mapMaxZ: number;
...
export interface MapTilesMessage {
  ...
  mapMinZ: number;
  mapMaxZ: number;
  lods: Array<{ level: number; width: number; height: number; tilesX: number; tilesY: number; path: string }>;
}
...
export interface MapSpriteData {
  spriteId: string;
  orthoSize: number;       // camera orthographic half-size
  pivotToCenterX: number;  // bounds.center.x - transform.position.x
  pivotToCenterY: number;  // bounds.center.y - transform.position.y
  pivotToCenterZ: number;  // bounds.center.z - transform.position.z
}
...
export interface DropSpawnMessage {
  type: 'drop_spawn';
  id: string;
  dropType: string;   // e.g. 'weapon', 'health', 'ammo', 'powerup'
  spriteId?: string;
  x: number;
  y: number;
  z: number;
...
export type ClientMessage =
  ...
  | MapTilesMessage
  | MapObjectsMessage
  | AbilityManifestMessage
  | WeaponManifestMessage
  | RouteLayoutMessage
  | PlayerSessionStatusMessage
