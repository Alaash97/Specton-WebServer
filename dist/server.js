...
const playerHandler_1 = require("./handlers/playerHandler");
const spectatorHandler_1 = require("./handlers/spectatorHandler");
const ObjectStorage_1 = require("./storage/ObjectStorage");
const rateLimiter_1 = require("./middleware/rateLimiter");
function createServer(port, rateLimitWindowMs, heartbeatIntervalMs) {
    const objectStorage = (0, ObjectStorage_1.createObjectStorage)();
    const sessionManager = new SessionManager_1.SessionManager(objectStorage);
    const clientStates = new Map();
    const spectatorRateLimiter = new rateLimiter_1.RateLimiter(rateLimitWindowMs);
    ...
    const httpServer = http_1.default.createServer(async (_req, res) => {
    ...
    wss.on('connection', (ws) => {
        const state = { role: 'unknown', isAlive: true, messageQueue: Promise.resolve() };
        ...
        ws.on('message', (raw) => {
            state.isAlive = true;
            const message = (0, validator_1.validateMessage)(raw.toString());
            if (!message) {
                const rawText = raw.toString();
                const reason = (0, validator_1.describeInvalidMessage)(rawText);
            ...
            if (state.role === 'player' && (message.type === 'create_session' || message.type === 'game_state' || message.type === 'map_image' || message.type === 'map_tile' || message.type === 'map_tiles' || message.type === 'map_objects' || message.type === 'ability_manifest' || message.type === 'weapon_manifest' || message.type === 'route_layout' || message.type === 'session_status' || message.type === 'projectile_spawn' || message.type === 'projectile_destroy' || message.type === 'drop_spawn' || message.type === 'drop_destroy' || message.type === 'damage_dealt' || message.type === 'finish_game')) {
                const runPlayerMessage = () => (0, playerHandler_1.handlePlayerMessage)(ws, message, sessionManager).catch((error) => {
                ...
                if (message.type === 'join_session' ||
                    ...
                    message.type === 'action') {
                    (0, spectatorHandler_1.handleSpectatorMessage)(ws, message, sessionManager, spectatorRateLimiter);
                }
            }
            else {
        ...
        ws.on('close', () => {
            if (state.role === 'player') {
                (0, playerHandler_1.handlePlayerDisconnect)(ws, sessionManager);
            }
            else if (state.role === 'spectator') {
                (0, spectatorHandler_1.handleSpectatorDisconnect)(ws, sessionManager);
            }
